...
public class Step4 {
	public int[] step4() {	
		...
		// 첫번째 자리에 1~45가 몇회차에 나왔는지 결과 도출
			if (script.get4(i).size() == 0) {
			//만약 한번도 나온적이 없는 숫자이면 다음 작업 수행
				continue;
			}
			//각 숫자의 마지막 출현회차 도출
			lasttime.put(i, script.getlast4(i));
			// 각 숫자의 통계적 출현 확률
			ratio = (float) (script.get4(i).size() / (200.00 / 100));
//			ratio = script.get4(i).size() / ((float)count.count() / 100);
		...
		for (int i = 1; i < 11; i++) {
			...
			ArrayList<Integer> lottoCount = script.get4(number);
			int avg = 0;
